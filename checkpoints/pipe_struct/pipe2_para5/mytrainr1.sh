...
EXP_DIR="/mrtstorage/users/zwang/experiments/m3l_b4_range1_1/"

cd "${CURRENT_DIR}"

mkdir -p "${EXP_DIR}/train"
cp "${WORK_DIR}"/mytrainr1.sh "${EXP_DIR}"/.
CUDA_VISIBLE_DEVICES=1 python "${WORK_DIR}"/train.py \
  ...
  --dataset_dir="/mrtstorage/users/zwang/pcd_mapper_pastonly/polar_single/train" \
  --tf_initial_checkpoint="/mrtstorage/users/zwang/experiments/m3l_b4_range1_1/model.ckpt"

   #--tf_initial_checkpoint="/mrtstorage/users/zwang/experiments/init/deeplab_m3l_cityscapes/model.ckpt"
